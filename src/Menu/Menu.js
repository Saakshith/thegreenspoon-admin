...
const Menu = () => {
  ...
  const handleAddItem = async (e) => {
    e.preventDefault()
    try{
      const res = await addDoc(collection(db, "menu-items"), {
        calories: calories, 
        cusine: cusine,
  ...
  const handleUpdateItem = async (id, e) => {
    ...
    try{
      console.log(id)
      const res = await setDoc(doc(db, "menu-items", id), {
          calories: calories, 
          cusine: cusine,
          description: description,
          image: imageURL,
  ...
  const handleDeleteItem = async (id, e) => {
    // e.preventDefault();
    try{
      const res = await deleteDoc(doc(db, "menu-items", id));
          ...
          <div className="menu-cards">
            ...
            {updateModal && 
              <Modal 
                ...
                handleItem={() => handleUpdateItem(id)}
                calories = {calories}
                cusine = {cusine}
                description = {description}
                imageURL = {imageURL}
